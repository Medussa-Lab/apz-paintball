...
const PLANS = [
  {
    ...
    features: ['Marcadora Tippmann 98 o BT-4', 'Máscara Valken MI-3 Thermal', 'Buzo integral + chaleco', 'Instructor/monitor', 'Seguro RC y Accidentes', 'Agua mineral'],
    featured: false,
    cta: 'Reservar',
  },
  {
  ...
  return (
    <section id="precios" className="py-32" ref={ref}>
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
          {PLANS.map((plan, i) => (
            <motion.div
              ...
              className={`relative flex flex-col rounded-tactical overflow-visible transition-all duration-300 ${
                plan.featured
                  ? 'bg-gradient-to-b from-[#181714] to-[#111110] border border-accent/30 shadow-[0_0_50px_rgba(255,208,0,0.08)] hover:shadow-[0_0_70px_rgba(255,208,0,0.15)]'
                  : 'bg-[#111110] border border-white/[0.06] hover:border-white/[0.14] hover:bg-[#141412]'
              }`}
            >
              ...
              {plan.isSpecial ? (
                <div className="flex flex-col gap-2 p-4 flex-1">
                  {INFANTIL_TIERS.map((tier, j) => (
                    <div key={j} className={`rounded-tactical px-4 py-3 ${tier.highlight ? 'bg-accent/8 border border-accent/25' : 'bg-black/20 border border-white/[0.04]'}`}>
                ...
                <ul className="flex flex-col gap-2.5 p-6 flex-1">
                  {plan.features.map((f, j) => (
                    <li key={j} className="flex items-start gap-2.5 text-[0.83rem] text-text/40 font-body">
                      <span className="text-accent flex-shrink-0 mt-0.5 text-xs">✓</span>{f}
                    </li>
                  ))}
              ...
              <div className="px-6 pb-6">
                <button
                  onClick={scrollTo}
                  className={`w-full font-display text-[0.78rem] tracking-[0.12em] uppercase py-3.5 rounded-tactical transition-all duration-200 ${
                    plan.featured
        ...
        >
          {EXTRAS.map((e) => (
            <div key={e.label} className="bg-[#0d0d0c] border border-white/[0.05] rounded-tactical px-5 py-4">
        ...
        <motion.div
          ...
          className="relative bg-accent/[0.04] border border-accent/15 rounded-tactical p-6 text-center overflow-hidden"
        >
